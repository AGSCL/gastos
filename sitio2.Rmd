...
fit_month_gasto_23 %>% 
dplyr::right_join(,fit_month_gasto_22,by="gasto") %>%
dplyr::right_join(fit_month_gasto_22,fit_month_gasto_21,by="gasto") %>% 
dplyr::right_join(fit_month_gasto_20,by="gasto") %>% 
...
forecast::forecast(arima_optimal,h=12)
...
```{r 2-5, echo=T, paged.print=TRUE,eval=T}
...
forecast_uf<-
cbind.data.frame(fecha=as.Date(seq(as.numeric(as.Date(uf_serie_corrected$date3[length(uf_serie_corrected$date3)])),(as.numeric(as.Date(uf_serie_corrected$date3[length(uf_serie_corrected$date3)]))+299),by=1)),forecast::forecast(fit_tbats, h=300)) %>% 
...
fr_fit_tbats_m2<-forecast::forecast(arima_optimal)
